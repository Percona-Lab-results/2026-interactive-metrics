...
REMOTE_BASE_LOG_DIR="benchmark_remote_logs" # Relative to $HOME
...
start_metrics() {
  ...
  # NOTE: dstate needs to be installed on the remote server manually 
  #       (sudo apt install dstat) and may require sudo permissions
  #       to run properly.
