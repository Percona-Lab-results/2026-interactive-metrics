...
for VERSION in "${VERSIONS[@]}"; do
  ./run_remote_metrics.sh "percona-server" "$VERSION" "$IS_READ_ONLY"
done

VERSIONS=("5.7" "8.0" "8.4" "9.6")
for VERSION in "${VERSIONS[@]}"; do
  ./run_remote_metrics.sh "mysql" "$VERSION" "$IS_READ_ONLY"
done


VERSIONS=("10.11" "11.4" "12.1")
for VERSION in "${VERSIONS[@]}"; do
  ./run_remote_metrics.sh "mariadb" "$VERSION" "$IS_READ_ONLY"
done
